...
if (!$pizza->find($id)) {
    ...
    echo json_encode(array("message" => "Pizza com id $id não existe."));
    exit;
}

if ($pizza->delete($id)) {
    echo json_encode(array("message" => "Pizza removida com sucesso.", "id" => $id));
} else {
    header("HTTP/1.1 500 Internal Server Error");
    echo json_encode(array("message" => "Erro ao excluir a pizza."));
}
